...
__all__ = ['ChaosModel', 'ExponFam', 'VRVM_PCE_Full']
...
class VRVM_PCE_Full(BaseEstimator):
	...
	def fit(self, X, Y):
		...
		if method == 'ascent':
			...
			while err > tol:
				...
				ETA = np.hstack([eta_tau.flatten(), eta_c.flatten()])
				#ETA_new = np.hstack([eta_tau_new.flatten(), eta_c_new.flatten(), eta_z_new ])
				ETA_new = np.hstack([eta_tau_new.flatten(), eta_c_new.flatten()])
